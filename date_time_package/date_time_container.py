...
class DateTimeContainer(object):
    """Класс-контейнер для объектов Дата/Время"""

    def __init__(self):
        """Конструктор контейнера. Формирует пустой list"""

        self.container = []

    def __str__(self):
        """Вывод данных контейнера в читаемом виде"""
    ...
    def add(self, date_time_object):
        ...
        if isinstance(date_time_object, dt.DateTime):
            ...
            print(
                "Вы пытаетесь добавить объект, не подходящий для хранения в"
                "данном контейнере")
    ...
    def json_save(self):
        ...
        with open('date_time_container.json', 'w', encoding='utf-8') \
                as export_file:
            json.dump(
                [date_time.__dict__ for date_time in self.container],
                export_file, indent=4)
